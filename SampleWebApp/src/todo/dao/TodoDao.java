...
public class TodoDao extends MySQLDAO {
	...
	public TodoValueObject detail(int id) throws Exception{
		TodoValueObject vo = new TodoValueObject();
		String sql = "SELECT id,title,task,limitdate,lastupdate, userid, label,td.status,filename FROM todo_list td LEFT JOIN status_list stts ON stts.status = td.status where id = ?";
		PreparedStatement statement = connection.prepareStatement(sql);
